...
est_boot <- function(df, p){
  ...
  vec_est3 <- c()
  for (c in 1:nboot) {
    vec_est1 <- append(vec_est1, (1000/9)*sum(unlist(df[,4 + (5*(c-1))])/p))
    vec_est2 <- append(vec_est2, (1000/9)*sum(unlist(df[,5*c])/p))
    vec_est3 <- append(vec_est3, ((1000/9)*sum(unlist(df[,5*c])/p))/((100/9)*sum(unlist(df[,4 + (5*(c-1))])/p)))
