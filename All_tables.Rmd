...
```{r}
tab_ci <- df_total %>%
  kable("latex", booktabs = TRUE, align = "llcccc", linesep = c("", "", "",
  "\\addlinespace"), caption = paste("Characteristics of confidence intervals when using ", B1, " Monte Carlo iterations and ", B2, " bootstrap replications")) %>% # \\hline
  kable_styling(latex_options = "hold_position") %>%
  row_spec(0, bold = TRUE) %>% # , hline_after = TRUE
  collapse_rows(columns = 1, latex_hline = "major")
```
