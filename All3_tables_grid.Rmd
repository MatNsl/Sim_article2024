...
In this document, \textbf{sampling through a grid} is used to create a sample and \textbf{three concentric circles} are used to compute local variables.
...
real_val <- c(nrow(trees), nrow(trees[(pi*trees$d130 < 70.5),]), nrow(trees[(pi*trees$d130 < 117.5) & (pi*trees$d130 > 70.5),]), nrow(trees[(pi*trees$d130 > 117.5),]), 
  ...
  round(sum(trees$v)/nrow(trees), digits = 2), round(sum(trees[(pi*trees$d130 < 70.5),]$v)/nrow(trees[(pi*trees$d130 < 70.5),]), digits = 2), round(sum(trees[(pi*trees$d130 < 117.5) & (pi*trees$d130 > 70.5),]$v)/nrow(trees[(pi*trees$d130 < 117.5) & (pi*trees$d130 > 70.5),]), digits = 2), round(sum(trees[(pi*trees$d130 > 117.5),]$v)/nrow(trees[(pi*trees$d130 > 117.5),]), digits = 2)
  )

cat("True integrals are the following:\n\nNumber of trees:", real_val[1], "\nTotal volume:", real_val[5], 
    "\nWhich leads to the following ratio \"Total volume / Number of trees\":", real_val[9],
    "\n\nNumber of small trees:", real_val[2], "\nVolume of small trees:", real_val[6], 
    "\nWhich leads to the following ratio \"Volume / Number of trees\":", real_val[10],
    "\n\nNumber of medium trees:", real_val[3], "\nVolume of medium trees:", real_val[7], 
    "\nWhich leads to the following ratio \"Volume / Number of trees\":", real_val[11],
    "\n\nNumber of big trees:", real_val[4], "\nVolume of big trees:", real_val[8], 
    "\nWhich leads to the following ratio \"Volume / Number of trees\":", real_val[12]
    )
```

```{r}
real_df <- data.frame(c("All trees", "Small trees", "Medium trees", "Big trees"), matrix(real_val, nrow = 4))
colnames(real_df) <- c("", "Number", "Volume", "Ratio Volume/Number")
```

```{r}
tab_real <- real_df %>%
  kable("latex", booktabs = TRUE, align = "lccc", linesep = c("", "\\addlinespace", "", "", "", ""), 
...
```{r}
tab_ci <- df_total %>%
  kable("latex", booktabs = TRUE, align = "llcccc", linesep = c("", "", "", "\\addlinespace", "", "", "", "\\addlinespace", "", "", "", "\\addlinespace", "", "", "", "\\hline"), caption = paste("Characteristics of confidence intervals when using ", B1, " Monte Carlo iterations and ", B2, " bootstrap replications")) %>% # \\hline
  kable_styling(latex_options = "hold_position") %>%
  row_spec(0, bold = TRUE) %>% # , hline_after = TRUE
  collapse_rows(columns = 1, latex_hline = "major")
```
...
```{r}
diff_percent <- function(j){unlist(100*(max(res1$Variances[j],VAR_df[j])-min(res1$Variances[j],VAR_df[j]))/min(res1$Variances[j],VAR_df[j]))}
...
diff_var <- data.frame(`Parameter of interest` = PARAM_names, `Difference between variances` = paste0(round(mapply(function(x,y){100*(max(x,y)-min(x,y))/min(x,y)}, VAR_df, res1$Variances), digits = 3), "%") )
```

```{r}
tab_var <- diff_var %>%
  kable("latex", booktabs = TRUE, align = "lc", linesep = c("", "", "",
