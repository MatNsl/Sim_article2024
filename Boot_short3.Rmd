---
title: "Testing some bootstrap process"
output: pdf_document
date: "2024-08-02"
---
...
Idea for the bootstrap: for each line l, containing $n_l$ sampled points, $n_l - 1$ points are sampled with replacement. As a result, $100(n_l-1)$ points are sampled from the initial sample. In this case, 900 points are sampled.
...
f_boot <- function(df_sample, B){
  ...
  for (b in 1:B) {
    ...
    for (number in unique(df_sample[,3])){
      names_h <- sample(rownames(df_sample[df_sample[,3] == number,]), (nrow(df_sample[df_sample[,3] == number,])-1), replace = TRUE)
      sample_h <- mapply(function(some_names){df_sample[rownames(df_sample) == some_names,]}, names_h)
      sample_b <- rbind(sample_b, t(sample_h))
    }
  ...
  return(df_boot)
  # Return a data frame with B samples of 5 variables
}
```
...
est_boot <- function(df, p){
  ...
  n <- nrow(df) + 100 # sample size of the initial sample
  nboot <- length(df)/5 # number of bootstrap samples
  vec_est1 <- c()
  vec_est2 <- c()
  vec_est3 <- c()
  for (c in 1:nboot) {
    vec_est1 <- append(vec_est1, (1/100)*(1000/9)*sum(as.vector(unlist(df[,4 + (5*(c-1))]))/p))
    vec_est2 <- append(vec_est2, (1/100)*(1000/9)*sum(as.vector(unlist(df[,5*c]))/p))
    vec_est3 <- append(vec_est3, ((1000/9)*sum(as.vector(unlist(df[,5*c])/p)))/((1000/9)*sum(as.vector(unlist(df[,4 + (5*(c-1))]))/p)))
