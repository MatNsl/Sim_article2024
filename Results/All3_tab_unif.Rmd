...
# Example: Uniform sampling with basic bootstrap

```{r}
# The adequate function and its name
k <- 1 # Uniform sampling
...
tab_rs <- DF_rs %>%
  ...
  "\\hline"), caption = paste0("Approximation of Relative Stability of bootstrap variance by using ", B1, " Monte Carlo iterations and ", B2, " bootstrap replications"), col.names = gsub("[.]", " ", names(DF_rs))) %>% # \\hline \\addlinespace
...
tab_var <- diff_var %>%
  kable("latex", booktabs = FALSE, align = "lc", linesep = c("", "", "",
  "\\addlinespace"), caption = paste0("Approximation of Relative Bias of bootstrap variance when using ", B1, " Monte Carlo iterations and ", B2, " bootstrap replications in one case and ", B0, " Monte Carlo iterations in the other case"), col.names = gsub("[.]", " ", names(diff_var))) %>% # \\hline
