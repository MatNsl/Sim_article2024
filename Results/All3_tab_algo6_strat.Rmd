...
In this document, \textbf{sampling through a grid} is used to create a sample, more precisely \textbf{algorithm 6}, and \textbf{three concentric circles} are used to compute local variables.


\textbf{Stratified bootstrap} is used.
...
# Example: Algorithm 6 with stratified bootstrap

```{r}
fun_samp <- sampling_grid1
```
...
res2 <- MC_boot(n, B1, B2, fun_samp, strat = TRUE)
# Just one circle to begin with
