...
diff_var <- data.frame(`Difference between totals` = paste0(round(mapply(function(x,y){unlist(100*(max(x,y)-min(x,y))/min(x, y))}, unlist(THEO_df[1]), unlist(RES_df[1])), digits = 3), "%"), `Difference between variances` = paste0(round(mapply(function(x,y){100*(max(x,y)-min(x,y))/min(x,y)}, unlist(THEO_df[2]), unlist(RES_df[2])), digits = 3), "%") )
