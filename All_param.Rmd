...
bcanon <- function(df_init,est_init,est_boot_val,p,nb_var,alpha =
    ...
    z0 <- mapply( function(j){qnorm(sum(mapply(function(long){long<est_init[,j]},est_boot_val[,j]))/nboot)} , 1:12) # length: nb_est
    # Same as: mapply(function(one, long){qnorm(sum(long<one)/nboot)}, t(est_init),est_boot_val)
      # qnorm(sum(mapply(function(one, long){long<one}, t(est_init),est_boot_val))/nboot)
    ...
    u <- cbind(u, mapply(function(j){mapply(function(x,y){x/y}, u[j+4], u[j])}, 1:3) )
...
est_boot <- function(df, p, nb_var){
  ...
  for (c in 1:nboot) {
    ...
    # as.data.frame(split(as.data.frame(big_est2), ","))
    # big_est2[c, j] <- 
    # big_est2[l_big_est+1] <- big_est2[c,1]/big_est2[c,1+4]
    }
  colnames(big_est) <- c("est_nb", "est_nb_s","est_nb_m", "est_nb_b", "est_vol", "est_vol_s", "est_vol_m", "est_vol_b", "est_ratio", "est_ratio_s", "est_ratio_m", "est_ratio_b")
...
MC_boot <- function(n, B1, B2){
  ...
  for (b in 1:B1) { # One MC iteration at a time
    ...
    nam_lim <- c("lim_nb", "lim_nb_s", "lim_nb_m", "lim_nb_b", 
                 "lim_vol", "lim_vol_s", "lim_vol_m", "lim_vol_b", 
                 "lim_ratio", "lim_ratio_s", "lim_ratio_m", "lim_ratio_b")
