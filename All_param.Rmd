...
f_boot <- function(df_sample, B, nb_est){
  ...
  for (b in 1:B) {
    ...
    colnames(df_boot)[((b-1)*(nb_var+2)+1 + 1):(b*(nb_var+2)+1)] <- as.vector(mapply(function(j){paste("ech", b, colnames(df_sample)[j], sep = "")}, 1:nb_est))
...
MC_boot <- function(n, B1, B2){
  ...
  for (b in 1:B1) { # One MC iteration at a time
    ...
    colnames(df_b)[((nb_var+2)*(b-1)+1):((nb_var+2)*(b)-1)] <- 
