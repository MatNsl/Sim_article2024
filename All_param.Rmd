...
MonteCarlo <- function(n, B){
  ...
  for (b in 1:B) { # All in one "for" loop
    ...
    var <- as.data.frame(t(mapply(circle_ALL_, xy$a1, xy$a2)))
    nb_var <- length(var) # number of variables of interest
    MC_df <- cbind(MC_df, xy, var)
    colnames(MC_df)[((nb_var+2)*(b-1)+2):((nb_var+2)*(b)+2-1)] <- c(paste("x", b, sep = ""), paste("y", b, sep = ""), paste("nb", b, sep = ""), paste("nb_s", b, sep = ""), paste("nb_m", b, sep = ""), paste("nb_b", b, sep = ""), paste("vol", b, sep = ""), paste("vol_s", b, sep = ""), paste("vol_m", b, sep = ""), paste("vol_b", b, sep = ""))
    from_local <- function(j){sum(MC_df[(nb_var+2)*(b-1)+3+j] /p)}
    est_b <- data.frame(from_local(1), from_local(5), (from_local(5))/(from_local(1)), from_local(2), from_local(6), (from_local(6))/(from_local(2)), 
                           from_local(3), from_local(7), (from_local(7))/(from_local(3)), from_local(4), from_local(8), (from_local(8))/(from_local(4)))
...
bcanon <- function(df_init,est_init,est_boot_val,p,nb_var,alpha =
    ...
    for(i in 1:n){
        ...
        u <- rbind(u, as.vector(mapply(function(j){mapply(sum(x_i[j]/p))}, 3:nb_var)))
    }
    # u <- cbind(u, mapply(function(x,y){x/y}, u[2], u[1]))
    u <- cbind(u, mapply(function(j){mapply(function(x,y){x/y}, u[j+4], u[j])}, 3:6) )
...
f_boot <- function(df_sample, B, nb_var){
  ...
  for (b in 1:B) {
    ...
    colnames(df_boot)[((b-1)*(nb_var+2)+1 + 1):(b*(nb_var+2)+1)] <- as.vector(mapply(function(j){paste("ech", b, colnames(df_sample)[j], sep = "")}, 1:(nb_var+2)))
      # c(paste("ech", b, colnames(df_sample)[1], sep = ""), paste("ech", b, colnames(df_sample)[2], sep = ""), paste("ech", b, colnames(df_sample)[3], sep = ""), paste("ech", b, colnames(df_sample)[4], sep = ""))
  }
...
est_boot <- function(df, p, nb_var){
  ...
  for (c in 1:nboot) {
    ...
    # (n/(n-1)) is the correction needed since we choose only (n-1) points
        ...
        # big_est2 <- mapply(function(vec_, j){
        # vec_[c] <- (n/(n-1))*sum(df[, 2 + j + ((nb_var+2)*(c-1))]/p)
        # return(as.data.frame(vec_))
        # }, big_est2, 1:nb_var) 
    # mapply(function(vec_, c){append(vec_, (n/(n-1))*sum(df[, 2 + c + ((nb_var+2)*(c-1))]/p))}, big_vec, 1:nboot)
    # vec_est1 <- append(vec_est1, (n/(n-1))*sum(df[,3 + ((nb_var+2)*(c-1))]/p))
    # vec_est2 <- append(vec_est2, (n/(n-1))*sum(df[,(nb_var+2)*c]/p))
    # vec_est3 <- append(vec_est3, ((n/(n-1))*sum(df[,(nb_var+2)*c]/p))/((n/(n-1))*sum(df[,3 + ((nb_var+2)*(c-1))]/p)))
  }
  ...
  for (c in 1:nboot) {
    # add_column()
    big_est2 <- mapply(function(j){big_est2[c, l_big_est+j] <- big_est2[c,j+4]/big_est2[c,j]
      return(as.data.frame(big_est2))
    }, 1) # 4 ratios
    # big_est2[l_big_est+1] <- big_est2[c,1]/big_est2[c,1+4]
    
    test <- function(j){big_est2[c, l_big_est+j] <- big_est2[c,j+4]/big_est2[c,j]     
    return(as.data.frame(big_est2))
    }
  }
  big_est <- t(big_est)
  colnames(big_est) <- c("est_nb", "est_nb_s","est_nb_m", "est_nb_b", "est_vol", "est_vol_s", "est_vol_m", "est_vol_b")
...
MC_boot <- function(n, B1, B2){
  ...
  for (b in 1:B1) { # One MC iteration at a time
    ...
    colnames(df_b)[((nb_var+2)*(b-1)+1):((nb_var+2)*(b))] <- 
      c(paste("x.MC", b, sep = ""), paste("y.MC", b, sep = ""), paste("nb.MC", b, sep = ""), paste("nb_s.MC", b, sep = ""), paste("nb_m.MC", b, sep = ""), paste("nb_b.MC", b, sep = ""), paste("vol.MC", b, sep = ""), paste("vol_s.MC", b, sep = ""), paste("vol_m.MC", b, sep = ""), paste("vol_b.MC", b, sep = ""))
    from_local <- function(j){sum(df_b[(nb_var+2)*(b-1)+2+j] /p)}
    est_b <- data.frame(from_local(1), from_local(5), (from_local(5))/(from_local(1)), from_local(2), from_local(6), (from_local(6))/(from_local(2)), 
                           from_local(3), from_local(7), (from_local(7))/(from_local(3)), from_local(4), from_local(8), (from_local(8))/(from_local(4)))
    nb_est <- length(est_b) # number of estimations
    rownames(est_b)[b] <- paste("ech", b, sep = "")
    ...
    nam_lim <- c("lim_nb", "lim_vol", "lim_ratio", "lim_nb_s", "lim_vol_s", "lim_ratio_s", 
                 "lim_nb_m", "lim_vol_m", "lim_ratio_m", 
                 "lim_nb_b", "lim_vol_b", "lim_ratio_b")
...
```{r}
cat("Monte Carlo means of estimations: \n - for the number of trees:", mean(as.vector(res1$Estimations[1,])), "\n - for the volume of trees:", mean(as.vector(res1$Estimations[2,])), "\n - for the ratio volume / number of trees:", mean(as.vector(res1$Estimations[3,])),
    "\n\n - for the number of small trees:", mean(as.vector(res1$Estimations[4,])), "\n - for the volume of small trees:", mean(as.vector(res1$Estimations[5,])), "\n - for the ratio volume / number of trees:", mean(as.vector(res1$Estimations[6,])),
    "\n\n - for the number of medium trees:", mean(as.vector(res1$Estimations[7,])), "\n - for the volume of medium trees:", mean(as.vector(res1$Estimations[8,])), "\n - for the ratio volume / number of trees:", mean(as.vector(res1$Estimations[9,])),
    "\n\n - for the number of big trees:", mean(as.vector(res1$Estimations[10,])), "\n - for the volume of big trees:", mean(as.vector(res1$Estimations[11,])), "\n - for the ratio volume / number of trees:", mean(as.vector(res1$Estimations[12,]))
    )
