...
bcanon <- function(df_init,est_init,est_boot_val,p,nb_est,alpha =
                     c(.025,.975)) { 
    ...
    for(i in 1:n){
        ...
        u <- rbind(u, as.vector(mapply(function(j){mapply(sum(x_i[j]/p))}, 1:nb_est)))
    }
...
f_boot <- function(df_sample, B, nb_est){
  ...
  for (b in 1:B) {
    ...
    colnames(df_boot)[((b-1)*4+1 + 1):(b*4+1)] <- as.vector(mapply(function(j){paste("ech", b, colnames(df_sample)[j], sep = "")}, 1:nb_est))
...
MC_boot <- function(n, B1, B2){
  ...
  for (b in 1:B1) { # One MC iteration at a time
    ...
    colnames(df_b)[((nb_var+2)*(b-1)+2):((nb_var+2)*(b)+2-1)] <- 
    ...
    from_local <- function(j){sum(df_b[(nb_var+2)*(b-1)+3+j] /p)}
    est_b <- data.frame(from_local(1), from_local(5), (from_local(5))/(from_local(1)), from_local(2), from_local(6), (from_local(6))/(from_local(2)), 
                           from_local(3), from_local(7), (from_local(7))/(from_local(3)), from_local(4), from_local(8), (from_local(8))/(from_local(4)))
    nb_est <- length(est_b) # number of estimations
    rownames(MC_est)[b] <- paste("ech", b, sep = "") 
    ...
    BCa_CI <- bcanon(df_b, est_b, est_boot_val,p, nb_est)
