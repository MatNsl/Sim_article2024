...
MonteCarlo <- function(n, B){
  ...
  for (b in 1:B) { # All in one "for" loop
    ...
    colnames(MC_df)[((nb_var+2)*(b-1)+2):((nb_var+2)*(b)+2-1)] <- paste0(raw_names, b)[-(11:14)]
...
MC_boot <- function(n, B1, B2){
  ...
  for (b in 1:B1) { # One MC iteration at a time
    ...
    BOOT_var[[b]] <- mapply(var, est_boot_val)
      # list(var_nb = var(est_boot_val$est_nb), var_vol = var(est_boot_val$est_vol), var_ratio = var(est_boot_val$est_ratio))
    ...
    test <- paste0("lim_", raw_names)[-(1:2)]
