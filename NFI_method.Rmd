...
```{r}
plot(x = random_points$x, y = random_points$y, xlab = "", ylab = "", main = "Random points", panel.first=grid())
```
...
```{r}
# Making of one estimation for each sample made by the bootstrap method
est_boot_nb <- est_boot(df_init, df_boot, circle3_nb)
```
...
```{r}
# Making of one estimation for each sample made by the bootstrap method
est_boot_vol <- est_boot(df_init, df_boot, circle3_vol)
```

```{r}
print("Two intervals for the volume of the forest:\n")
res_nb <- ci_perc(est_init_nb,est_boot_vol)
```

```{r}
# Visualizing the density, the interval and the first estimation
plot(density(est_boot_nb), xlab="Volume of the forest",ylab="", main = "Distribution based on bootstrap", las = 1)
