...
coverage_rate <- function(nb){
  ...
  for (elt in res2$`Confidence Intervals`) {
    CI_nb <- cbind(CI_nb, c(unlist(res2$`Confidence Intervals`$CI1[1])[1], unlist(res2$`Confidence Intervals`$CI1[1])[2]))
    CI_vol <- cbind(CI_nb, c(unlist(res2$`Confidence Intervals`$CI1[1])[3], unlist(res2$`Confidence Intervals`$CI1[1])[4]))
    CI_ratio <- cbind(CI_nb, c(unlist(res2$`Confidence Intervals`$CI1[1])[5], unlist(res2$`Confidence Intervals`$CI1[1])[6]))
  }
  CI_nb$useless <- NULL
...
cat("Concerning the normal approach, the coverage rates are the following: \n - for the number of trees: ", 
    ...
    " \n \nThe mean length of coverage rates are the following: \n - for the number of trees: ",
