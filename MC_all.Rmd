...
MonteCarlo <- function(n, B){
  ...
  return(list(Data = MC_df, Estimators = MC_est, Variances = MC_var))
  # Data: local variables (number and voume) for each point in each sample
...
```{r}
# mean(as.vector(res_MC$Estimators[1,]))
```

```{r}
# mean(res_MC$Estimators[1,])
```
...
```{r}
cat("Monte Carlo means of estimators: \n - for the number of trees:", mean(as.vector(res1$Estimators[1,])), "\n - for the volume of trees:", mean(as.vector(res1$Estimators[1,])), "\n - for the ratio volume / number of trees:", mean(as.vector(res1$Estimators[1,])))
# Example:
```

```{r}
cat("Monte Carlo variance: \n - for the number of trees:", res1$Variances[1], "\n - for the volume of trees:", res1$Variances[2], "\n - for the ratio volume / number of trees:", res1$Variances[3])
# Example:
```
...
cat("We get a difference of: ", diff_percent(), "%", sep = "")
# Examples:
