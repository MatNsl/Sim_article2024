...
comparison2 <- function(B) {
  ...
  while (b1  < B) {
    ...
    random_points_1 <- random_points
    random_points_1$l <- NULL
    df_boot <- f_boot(random_points_1, b1)
    est_boot_1 <- est_boot1(df_boot)
    
    ## Second method
    df_boot2 <- f_boot2(random_points, b1)
...
p2 <- ggplot(data, aes(data[,1], data[,3]))
p1 + p2 + geom_point()
p1 + geom_point

# p <- ggplot(data, aes(data[,1], cbind(data[,2], data[,3])))
# p + geom_point()

p <- ggplot(data, aes(data[,1], data[,3] - data[,2]))
p + geom_point()
```
...
p <- ggplot(data_var, aes(data_var[,1], data_var[,3] - data_var[,2]))
p + geom_point()
```
...
```{r}
ggplot(x = 1:length(res[[1]]), y = res[[1]]) +
  plot(x = 1:length(res[[1]]), y = res[[2]], col = "red")
geom_point(
