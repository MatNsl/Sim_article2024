...
10 000 Monte Carlo iterations would be fine
...
MonteCarlo <- function(n, B, f){
  ...
  for (b in 2:B) {
    ...
    series_MC <- series_MC %>% add_column(x = a1, y = a2, z = a)
    col_names <- append(col_names, c(x, y, z))
  }
  series_MC <- series_MC[,-1]
...
MC1 <- MonteCarlo(n, 10000, circle_nb)
...
```{r}
cat("Monte Carlo variance:", MC1[2])
```
...
1 000 Monte Carlo iterations should be fine
400 bootstrap iterations for each would be fine (200 to begin with if it is too much)
...
f_boot <- function(df_sample, B){
  ...
  for (b in 1:B) {
    nb <- sample(1:(nrow(df_sample)-1), nrow(df_sample), replace = TRUE)
...
res <- MC_boot(n, B1 = 1000, B2 = 400, f = circle_nb)
# Just one circle to begin with
...
```{r}
cat("Mean of the variance of bootstrap samples:", res[3])
```
