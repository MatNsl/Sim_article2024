...
MonteCarlo <- function(n, B){
  ...
  for (b in 1:B) { # All in one "for" loop
    ...
    colnames(MC_df)[(5*(b-1)+2):(5*(b-1)+2+4)] <- c(paste("x", b, sep = ""), paste("y", b, sep = ""), paste("h", b, sep = ""), paste("nb", b, sep = ""), paste("vol", b, sep = ""))
...
f_boot <- function(df_sample, B){
  ...
  for (b in 1:B) {
    ...
    for (number in unique(df_sample[,3])){
      truc <- sample(df_sample[df_sample[,3] == number,], nrow(df_sample[df_sample[,3] == number,]) -1, replace = TRUE)
      sample_b <- rbind(sample_b, truc)
...
est_boot <- function(df, p){
  ...
  vec_est3 <- c()
  for (c in 1:nboot) {
    vec_est1 <- append(vec_est1, (900/899)*sum(df[,4 + (5*(c-1))]/p))
    vec_est2 <- append(vec_est2, (900/899)*sum(df[,5*c]/p))
    vec_est3 <- append(vec_est3, ((900/899)*sum(df[,5*c]/p))/((900/899)*sum(df[,4 + (5*(c-1))]/p)))
