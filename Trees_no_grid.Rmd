...
A sample is created by uniformly sampling the whole territory except the edge, which depends on the radius (to facilitate the calculation of the local variable).
...
```{r}
# Uniform sampling over the whole territory except the edge
random_points$x <- runif(n, 0+R, 1000-R)
random_points$y <- runif(n, 0+R, 1000-R)
```
...
```{r}
# Creating a local variable (pages 10-11 in Trinh et alii (2024))
local_var <- c()
for (placette in 1:n) {
  local_var <- append(local_var, sum(trees_in[trees_in[placette] != 0,]/area_circle))
}
...
true_vs_MC <- function(n, B, R){
  ...
  series_MC <- data.frame(x.1 = runif(n, 0+R, 1000-R), y.1 = runif(n, 0+R, 1000-R))
  col_names <- c("x1", "y1")
  for (b in 2:B) {
    x <- paste("x", b, sep = "")
    y <- paste("y", b, sep = "")
    # Uniform sampling over the whole territory except the edge
    a1 <- runif(n, 0+R, 1000-R)
    a2 <- runif(n, 0+R, 1000-R)
  ...
  true_value <- nrow(trees) # Theoretical value
