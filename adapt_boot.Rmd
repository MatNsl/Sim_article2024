...
comparison <- function(ncomp, B) {
  ...
  for (b in 1:ncomp) {
    ...
    tot_mean <- append(tot_mean, as.numeric(abs(mean(est_boot_2)-30942) <  abs(mean(est_boot_1)-30942)))
    # 1: the second method is better
    tot_var <- append(tot_var, as.numeric(var(est_boot_2) <  var(est_boot_1)))
    # 1: the second method is better
    b <- b+1
  }
