...
bcanon <- function(x,bootsam,theta,...,alpha =
    ...
    a <- c()
    for (i in 1:n) {
      a <- append(a, circle3_nb(x[i, 1], x[i, 2], 15, 9, 6))
    }
    # Estimated total thus obtained from the initial sample
    thetahat <- sum(a/p)
    ...
    for (c in 1:B) {
      a <- c()
      for (i in 1:n) {
        a <- append(a, theta(bootsam[i, (2*c)-1], bootsam[i, 2*c], 15, 9, 6))
      }
    ...
    for(i in 1:n){
        ...
        a <- c()
        for (j in 1:n) {
          a <- append(a, theta(x_i[j, 1], x_i[j, 2], 15, 9, 6))
        }
        # Estimated total thus obtained from the initial sample
        u[i] <- sum(a/p)
    }
...
```{r}
bcanon(random_points,bootsam,circle_nb,...,alpha = .95)
```
