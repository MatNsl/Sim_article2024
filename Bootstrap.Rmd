...
source("~/work/Sim_article2024/Circle.R")
...
#   col_ <- paste("est", c, sep = "")
  ...
  a <- c()
  for (i in 1:n) {
      a <- append(a, circle(random_points[i, 1], random_points[i, 2], R))
    }
  # a <- circle(df_boot[(2*c)-1], df_boot[2*c], R)
  # print(col_)
...
```{r}
res_boot <- boot(data=random_points, statistic=circle)
res_boot
# Doesn't work
```
